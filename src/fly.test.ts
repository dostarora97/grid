...
const TUNE: Pick<FlyTune, 'deadzonePx' | 'radiusPx' | 'maxSpeedPx' | 'curveExp'> = {
  deadzonePx: 10,
  radiusPx: 110,
  maxSpeedPx: 1000,
  curveExp: 2,
...
describe('flyVelocity', () => {
  it('is exactly zero at center and inside the dead-zone', () => {
    expect(flyVelocity(0, 0, TUNE)).toEqual([0, 0]);
    expect(flyVelocity(9, 0, TUNE)).toEqual([0, 0]);
    expect(flyVelocity(0, -10, TUNE)).toEqual([0, 0]); // on the dead-zone edge
  });

  it('reaches max speed at (and beyond) the clamp radius', () => {
    const [vx] = flyVelocity(110, 0, TUNE);
    expect(vx).toBeCloseTo(1000, 6); // norm = 1 → full speed
    const [vx2] = flyVelocity(500, 0, TUNE);
  ...
  it('ramps as norm^curveExp between dead-zone and radius', () => {
    ...
    const [vx] = flyVelocity(60, 0, TUNE);
    expect(vx).toBeCloseTo(250, 6);
  });

  it('aims along the stick direction (unit vector × speed)', () => {
    const [vx, vy] = flyVelocity(60, 60, TUNE);
    // mag = 84.85; norm = (84.85-10)/100 = 0.7485; speed = 1000·0.7485² ≈ 560.3.
  ...
  it('carries the sign of the stick (all four directions)', () => {
    expect(flyVelocity(-60, 0, TUNE)[0]).toBeLessThan(0);
    expect(flyVelocity(0, -60, TUNE)[1]).toBeLessThan(0);
    expect(flyVelocity(0, 60, TUNE)[1]).toBeGreaterThan(0);
  });
});
